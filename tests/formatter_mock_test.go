...
func TestFormatterMock_ImplementsStringer(t *testing.T) {
	...
	// this is not required since tester registers MinimockFinish as a cleanup function
	// but we want to make sure that existing tests that use MinimockFinish don't break
	defer tester.MinimockFinish()

	v := NewFormatterMock(tester)
...
func (dmc *dummyMockController) Cleanup(f func()) {
	f()
}
