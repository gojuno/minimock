...
type GenericInMockNameExpectation[T any] struct {
	...
	expectationOrigins GenericInMockNameExpectationOrigins
...
type GenericInMockNameExpectationOrigins struct {
	origin   string
	originT1 string
}
...
func (m *GenericInMock[T]) MinimockNameInspect() {
	for _, e := range m.NameMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Errorf("Expected call to GenericInMock.Name at\n%s with params: %#v", e.expectationOrigins.origin, *e.params)
		}
	}
	...
	if !m.NameMock.invocationsDone() && afterNameCounter > 0 {
		m.t.Errorf("Expected %d calls to GenericInMock.Name at\n%s but found %d calls",
