...
const (
	...
	HeaderTemplate = `
        ...
        {{if $.Options.HeaderVars.TestSuffixPackageName}}
           {{ $packageName = printf "%s_test" $packageName }}
        {{end}}
		package {{$packageName}}
		...
		{{if $.Options.HeaderVars.GenerateInstruction}}
           {{ $gen := printf "//go:generate minimock -i %s.%s -o %s -n %s" $.SourcePackage.PkgPath $.Options.InterfaceName $.Options.OutputFile (title (index $.Vars "MockName")) }}
           {{if $.Options.HeaderVars.TestSuffixPackageName}}
              {{ $gen = printf "%s -t" $gen }}
           {{end}}
           {{ $gen }}
